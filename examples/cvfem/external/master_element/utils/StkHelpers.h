  ...
  const stk::mesh::EntityRank entity_rank = stk::topology::NODE_RANK)
{
  ...
  STK_NGP_ThrowRequireMsg((field != nullptr),
                          "Requested field does not exist: " + fieldName);
  ...
  const stk::mesh::EntityRank entity_rank = stk::topology::NODE_RANK)
{
  ...
  STK_NGP_ThrowRequireMsg((field != nullptr),
                          "Requested field does not exist: " + fieldName);
  STK_NGP_ThrowRequireMsg((field->is_state_valid(state)),
                          "Requested invalid state: " + fieldName);

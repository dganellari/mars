 ...
 *
 *  Nalu wrapper to provide SIMD capability for vectorizing nalu::Kernel
 *  algorithms.
 */
...
inline int get_length_of_next_simd_group(int index, int length)
{
    ...
    {
        std::cout << "ERROR, nextLength=" << nextLength << " shouldn't happen!!"
                  << std::endl;
        nextLength = 0;
    }
    return nextLength;
...
typedef accel::DoubleType DoubleType;

    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_CLASS_LAMBDA( // class lambda for this->numNodes_
                ...
                [&](const int& iElement)
            {
                ...
                stk::mesh::Entity connectedNodes[nodesPerElementMax];
                ...
                {
                    meSCS->shifted_grad_op(ws_coordinates, ws_dndx, ws_deriv);
                }
                else
                {
                ...
                for (label ip = 0; ip < numScsIp; ++ip)
                {
                    ...
                    {
                        const scalar r = ws_shape_function(ip, ic)._data.get();
                        ...
                        for (unsigned i = 0; i < SPATIAL_DIM; ++i)
                        {
                            coordIp[i] += r * ws_coordinates(ic, i)._data.get();
                        }
                    }
                    ...
                    for (unsigned i = 0; i < BLOCKSIZE; ++i)
                    {
                        ...
                        const label rowL = indexL * nodesPerElement * BLOCKSIZE;
                        const label rowR = indexR * nodesPerElement * BLOCKSIZE;
                        ...
                        for (unsigned i = 0; i < BLOCKSIZE; ++i)
                        {
                            ...
                            for (unsigned j = 0; j < SPATIAL_DIM; ++j)
                            {
                                ...
                                const scalar lhsfacDiff_i =
                                    -GammaIp * ws_dndx(ip, ic, j)._data.get() *
                                    axj;
                ...
                applyCoeff_(
                    A, b, connectedNodes, rhs, lhs, iElement, nodesPerElement);
            });
        });
...
template <typename scalar,
          ...
          bool includeAdv,
          bool isShifted>
KOKKOS_FUNCTION void AsmCvFemTeamScratch<scalar,
                                         label,
                                         BLOCKSIZE,
                                         SPATIAL_DIM,
                                         includeAdv,
                ...
                bool deductUnfound) const
{
    ...
    label scratchIds[nodesPerElementMax];
    label matrixColumnIds[nodesPerElementMax];
    label sortPermutation[nodesPerElementMax];
...
template class AsmCvFemTeamScratch<double, int, 1, 3, true, true>;
template class AsmCvFemTeamScratch<double, int, 3, 3, true, true>;

...
namespace mars {
    ...
    template <Integer Type>
    MARS_INLINE_FUNCTION void get_elem_coordinates(const DMQ2 &dm, const Integer elem_index, double *points) {
        SFC<Type> dof = dm.get_local_dof_enum();
        ...
        for (int i = 0; i < DMQ2::elem_nodes; i++) {
            ...
            get_vertex_coordinates_from_sfc<Type>(
                sfc_elem, points + 3 * i, dof.get_XDim(), dof.get_YDim(), dof.get_ZDim());
        }
    }
    ...
    void integrate(const DMQ2 &dm,
                   const FEQuad4<double>::Quadrature &quad,
                   ViewVectorType<double> det_J,

...
class UMeshLaplace final : public UMeshOperator<Mesh> {
 public:
  ...
  //     Kokkos::parallel_for(
  //         "UMeshLaplace::apply", mesh.n_elements(), MARS_LAMBDA(const Integer
  //         i) {
  //             Real u[NFuns];
  //             Real Au[NFuns];
  //             Integer idx[NFuns];
  ...
  //             SimplexLaplacian<Mesh>::one_thread_eval(J_inv_e, det_J(i), u,
  //             Au);
  ...
  class JacobiPreconditioner final : public UMeshJacobiPreconditioner<Mesh> {
   public:
    void init(FEValues<Mesh> &values) override {
      ...
      Kokkos::parallel_for(
          "JacobiPreconditioner::init", mesh.n_elements(),

// mirage JVP gate (p=1): the GENERATED Jacobian-action kernel J(u).du vs a
...
// host proof in mirage-compiler/tests/validate_jvp.py. F and J are BOTH mirage-
// generated (mirage_nldemo_apply.cuh); the check is self-contained.
//
// nldemo is a differentiation-machinery test operator, not physics. Setup
// (domain/elemDof/metric d_G) mirrors mars_mirage_ho_apply.cu.
...
#include "backend/distributed/unstructured/mirage/generated/mirage_nldemo_apply.cuh"
...
int main(int argc, char** argv)
{
    ...
    // Hand banks (xi/zeta) feed the metric kernel; mirage banks feed nldemo.
    CK(ho_cvfem_upload_operators(1, op.Btil.data(), op.Dtil.data(), op.D.data(),
                                 op.W.data(), op.xi.data(), op.zeta.data()));
    CK(mars::mirage::nldemo_upload_operators(1, op.Btil.data(), op.Dtil.data(),
                                             op.D.data(), op.W.data()));
    ...
    CK((mars::mirage::nldemo_apply_launch<RealType, 1>(d_up.data(), d_Fp.data(), d_elemDof.data(), d_G.data(), elementCount)));
    CK(cudaMemset(d_Fm.data(), 0, numTotalDofs*sizeof(RealType)));
    CK((mars::mirage::nldemo_apply_launch<RealType, 1>(d_um.data(), d_Fm.data(), d_elemDof.data(), d_G.data(), elementCount)));
    CK(cudaMemset(d_J.data(), 0, numTotalDofs*sizeof(RealType)));
    CK((mars::mirage::nldemo_jvp_apply_launch<RealType, 1>(d_u.data(), d_du.data(), d_J.data(), d_elemDof.data(), d_G.data(), elementCount)));
    cudaDeviceSynchronize();
    ...
    bool ok = rel < 1e-6;
    printf("mirage JVP gate (nldemo, p=1): elems=%zu dofs=%d  J_vs_FD_rel=%.3e  %s\n",

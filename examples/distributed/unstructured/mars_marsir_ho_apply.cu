// marsir parity gate (p=1): the generated Laplacian apply vs the hand-written
// mars::fem::ho_cvfem_apply_kernel, on the SAME mesh, elemDof, and metric d_G.
//
// The generated kernel (backend/.../marsir/generated/marsir_laplacian_apply.cuh) is
// emitted by marsir-compiler from specs/laplacian.op. It shares the hand kernel's
// idx layout, __constant__ operator banks (own names, marsir_c_*), and 4-step
...
// oracle by marsir-compiler/tests/validate_host.py.
//
...
#include "backend/distributed/unstructured/marsir/generated/marsir_laplacian_apply.cuh"
...
__constant__ int c_hexCornerToHoLocal_marsir[8] = {0, 4, 6, 2, 1, 5, 7, 3};

template<typename KeyType, typename RealType>
__global__ void marsir_p1_extract_kernel(
    ...
    RealType* __restrict__ d_corners, int* __restrict__ d_elemDof)
{
    ...
    for (int c = 0; c < 8; ++c) {
        ...
        d_elemDof[e * 8 + c_hexCornerToHoLocal_marsir[c]] = d_nodeToDof[nd];
    }
}
...
int main(int argc, char** argv)
{
    ...
    // Reference operators -> BOTH constant-memory banks (hand c_*, generated marsir_c_*).
    ...
    CK(mars::marsir::laplacian_upload_operators(1, op.Btil.data(), op.Dtil.data(),
                                             op.D.data(), op.W.data()));
    ...
    cstone::DeviceVector<int>      d_elemDof(elementCount * 8);
    {
        ...
        marsir_p1_extract_kernel<KeyType, RealType><<<grid, blk>>>(
    ...
    CK((mars::marsir::laplacian_apply_launch<RealType, 1>(d_u.data(), d_ymir.data(), d_elemDof.data(),
                                                       d_G.data(), elementCount)));
    cudaDeviceSynchronize();
    ...
    CK((mars::marsir::laplacian_apply_launch<RealType, 1>(d_uc.data(), d_yc.data(), d_elemDof.data(),
                                                       d_G.data(), elementCount)));
    cudaDeviceSynchronize();
    ...
    printf("marsir parity (p=1): elems=%zu dofs=%d  gen_vs_hand_rel=%.3e  nullspace=%.3e  %s\n",
           elementCount, numDofs, rel, nullmax, ok ? "PASS" : "FAIL");

    // ---- Throughput: generated MARSIR kernel vs hand kernel, same work ----
    ...
    auto marsirOp = [&]() {
        ...
        mars::marsir::laplacian_apply_launch<RealType, 1>(d_u.data(), d_ymir.data(), d_elemDof.data(), d_G.data(), elementCount);
    };
    for (int w = 0; w < 5; ++w) { handOp(); marsirOp(); }   // warmup
    CK(cudaGetLastError()); cudaDeviceSynchronize();
    ...
    double marsirMs = bench(marsirOp);
    double handTP = numDofs / (handMs * 1e-3);       // DOF/s
    double marsirTP = numDofs / (marsirMs * 1e-3);
    printf("marsir perf  (p=1): iters=%d  hand=%.3f ms (%.3e dof/s)  marsir=%.3f ms (%.3e dof/s)  marsir/hand=%.3f\n",
           iters, handMs, handTP, marsirMs, marsirTP, marsirMs / handMs);

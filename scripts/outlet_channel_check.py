...
def read_run(text):
    ...
    config = configs[0]
    required = {'ranks', 'steps', 'dt', 'nu', 'rho', 'inlet', 'ramp_steps', 'beta', 'p_ref',
                'empty_opening_ranks'}
    ...
    for step, record in enumerate(records, 1):
        ...
        if (abs(record['q_in']+record['q_out']) > 1e-8 or record['q_out'] <= 0
                or abs(record['q_in']+config['inlet']*ramp) > 1e-10
...
def compare(texts, require_empty=False):
    ...
    for config, records in runs[1:]:
        for key in ('steps', 'dt', 'nu', 'rho', 'inlet', 'ramp_steps', 'beta', 'p_ref'):

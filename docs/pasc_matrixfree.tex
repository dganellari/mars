...
\begin{frame}{Performance \& memory vs.\ the assembled kernel}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.52\textwidth}
      \begin{figure}
      % Two SEPARATE plots stacked (groupplot rendered as overlapping strips
      % here; separate tikzpictures + \vspace is the reliable layout).
      \begin{tikzpicture}
        \begin{axis}[marsplot, height=0.30\linewidth,
            title={Matrix-free throughput}, ylabel={MDOF/s},
            xmin=0.5,xmax=7.5, xtick={1,2,3,4,5,6,7}, ymin=0, ymax=9000]
          \addplot[marsline,color=marsblue] coordinates {
      ...
      \vspace{0.4em}
      \begin{tikzpicture}
        \begin{axis}[marsplot, height=0.30\linewidth, ymode=log,
            title={Memory footprint}, xlabel={polynomial order $p$},
            ylabel={bytes / DOF}, xmin=0.5,xmax=7.5, xtick={1,2,3,4,5,6,7},
            ymin=80, ymax=60000, legend pos=north west]
          \addplot[marsline,color=marsorange] coordinates {
            (1,324)(2,1500)(3,4116)(4,8748)(5,15972)(6,26364)(7,40500)};
          \addlegendentry{assembled}
          \addplot[marsline,color=marsblue] coordinates {
...
\begin{frame}{Scaling \& outlook}
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item Procedural per-rank mesh gen -- no mesh file
        \item Path to $10^{9}$--$10^{12}$ DOF
              ($\sim$1550 GH200 at the 647M-DOF/GPU $p{=}4$ ceiling)
      \end{itemize}
    ...
    \begin{column}{0.52\textwidth}
      ...
      \begin{tikzpicture}
        ...
        \begin{loglogaxis}[marsplot, height=0.32\linewidth,
            ...
            log basis x=10]
          % 647M DOF/GPU (p=4 ceiling) -> 1e12 at ~1545 GPUs
          \addplot[marsline,color=marsblue,mark=none] coordinates {
            (1,6.47e8)(10,6.47e9)(100,6.47e10)(1000,6.47e11)(1545,1e12)};
          \addplot[only marks,mark=*,mark size=1.5pt,color=marsorange]
            coordinates {(1545,1e12)};
          \node[font=\scriptsize,color=marsorange,anchor=south east]
            at (axis cs:1545,1e12) {$10^{12}$};
        \end{loglogaxis}
      \end{tikzpicture}
      \end{figure}
    \end{column}
  \end{columns}
  {\scriptsize\textcolor{marsgrey}{\textbf{Next:} the solve (FlexGMRES + LOR-AMG)
   $\;\cdot\;$ conforming-unstructured $10^{9}$ DOF via uniform refinement of an unstructured base}}
  \logostrip
  \note{Close on scale and what sets MARS apart. The efficiency plot is a
    ...
    projection line is the trillion-DOF narrative: at the measured 647M-DOF/GPU
    p=4 ceiling, ~1550 GH200 reach 1e12 DOF (projected, NOT a measured run --
    say "projected ~1,550 GPUs"), enabled by procedural per-rank mesh
    ...
    structured-AMR overset approach in Nalu-Wind / Kynema-UGF. Close on the
    "Next" beat as ROADMAP, not results: the solve (FlexGMRES + LOR-AMG on top
    of the validated operator) and a genuinely-unstructured demonstration
    (uniform refinement of an unstructured base -> conforming 1e9-DOF mesh with
    irregular valence/partitioning, vs today's block-hex topology). Say "next",
    not "done".}
\end{frame}

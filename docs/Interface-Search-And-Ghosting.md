...
**Incremental updates** (`update(add, remove)`) edit the ghosting in place rather than rebuilding
it, which is what a sliding interface needs — it re-pairs every timestep.

The enabling decision is in `build()`: each peer's lists are sorted **by key**, not by local index.
Slot *j* therefore means "the *j*-th smallest key I share with this peer", so both sides stay
aligned without any position ever going on the wire, and an edit is a linear merge rather than a
renumbering of everything after the insertion point. STK keeps its comm list sorted by `EntityKey`
for exactly this reason.

Removal is **communicated, not local bookkeeping**: if a rank drops a ghost and the owner keeps
sending it, the per-peer send and recv counts disagree and the next exchange misaligns — silent
corruption rather than a clean failure. STK does the same
(`BulkData::internal_change_ghosting` -> `comm_sync_send_recv`). Ours takes ONE round where
`build()` takes two: the payload is self-describing and `MPI_Probe` gives the size.

Gated by `IncrementalMatchesRebuild`: a random delta sequence through `update()` must equal
`build()` of the same final set — peers, counts, send/recv lists, and the actual forward and
reverseAdd results, element-for-element.
...
- **The local overlap test in the coarse search is brute force**, O(nQ x nRange) per peer. Fine for
...
- **The incremental path has no device gate yet.** `update()` invalidates the upload so the GPU
  index lists and the `reverseAdd` inverse CSR rebuild lazily, but that is argued, not measured --
  `mars_search_ghost_mpi.cu` should exercise an `update()` before comparing device against host.

...
namespace mars {
    ...
    public:
        ...
        MARS_INLINE_FUNCTION Point(SubView<T, Dim> pv) : pointView(pv) {}

        MARS_INLINE_FUNCTION Point(const ViewMatrixType<T> &v, Integer id) : Point(SubView<T, Dim>(&v, id)) {}
        ...
        bool operator<(const Point &right) const {
            for (Integer i = 0; i < Dim; ++i) {
                if (this->operator[](i) < right[i]) return true;
                if (this->operator[](i) > right[i]) return false;
            }
        ...
        T squared_distance(const Point &right) const {
            T sqn = (this->operator[](0) - right[0]) * (this->operator[](0) - right[0]);
            for (Integer i = 1; i < Dim; ++i) {
                sqn += (this->operator[](i) - right[i]) * (this->operator[](i) - right[i]);
            }
            return sqn;
        }

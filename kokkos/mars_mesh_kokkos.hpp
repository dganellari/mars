...
*/
        ...
        MARS_INLINE_FUNCTION
        const UnorderedMap<Integer, Integer> &get_elem_children_map() const { return elem_children_map_; }
        ...
        MARS_INLINE_FUNCTION
        void set_n_childrens(Integer size) { children_size_ = size; }

        MARS_INLINE_FUNCTION
        Integer n_elements() const override { return elements_size_; }

        MARS_INLINE_FUNCTION
        Integer n_childrens() const { return children_size_; }

        MARS_INLINE_FUNCTION
        void set_n_elements(Integer size) { elements_size_ = size; }

        Integer n_active_elements() const {
            Integer result = 0;
            ViewVectorType<bool> active = get_view_active();

            Kokkos::parallel_reduce(
                "Mesh::n_active_elements",
                active_.extent(0),
                KOKKOS_LAMBDA(const int &i, Integer &lsum) { lsum += active(i); },
                result);
        ...
        /*bool have_common_sub_surface(
         ...
         std::cerr << "common face not matching for (" << i << ", " << k << ") and (" << j << ", " << other_side_index
         << ")" << std::endl; std::cerr << "[ "; for(auto s : side.nodes) { std::cerr << s << " ";
         }
         std::cerr << " ]\n";
        ...
        struct AddCenterHexPoint {
            ...
            AddCenterHexPoint(ViewMatrixType<Real> pts, Integer xdm, Integer ydm, Integer zdm)
            ...
            void operator()(int z, int y, int x) const {
                ...
                points(centerHex, 0) = static_cast<Real>(2 * x + 1) / static_cast<Real>(2 * xDim);
                points(centerHex, 1) = static_cast<Real>(2 * y + 1) / static_cast<Real>(2 * yDim);
                points(centerHex, 2) = static_cast<Real>(2 * z + 1) / static_cast<Real>(2 * zDim);
            }
        };
        ...
        struct AddPoint {
            ...
            AddPoint(ViewMatrixType<Real> pts, Integer xdm) : points(pts), xDim(xdm) {}

            AddPoint(ViewMatrixType<Real> pts, Integer xdm, Integer ydm) : points(pts), xDim(xdm), yDim(ydm) {}
            ...
            void operator()(int z, int y, int x) const {
                ...
                /*
                                         ...
                                         The multiple of 2 indexes are the ones to be added to the mesh. The others
                   discarded.
                */
                if (in % 2 == 0) {
                    Integer i = in / 2;

                    points(i, 0) = static_cast<Real>(x) / static_cast<Real>(2 * xDim);
                    points(i, 1) = static_cast<Real>(y) / static_cast<Real>(2 * yDim);
                    points(i, 2) = static_cast<Real>(z) / static_cast<Real>(2 * zDim);
                }
            }
        };
        ...
        inline bool generate_points(const int xDim, const int yDim, const int zDim) {
            ...
            switch (ManifoldDim_) {
                ...
                case 2: {
                    ...
                    parallel_for(MDRangePolicy<Rank<2>>({0, 0}, {xDim + 1, yDim + 1}), AddPoint(points_, xDim, yDim));
                ...
                case 3: {
                    ...
                    const int n_tetra_nodes = 5 * (xDim * yDim * zDim) + 2 * (xDim * yDim + xDim * zDim + yDim * zDim) +
                                              (xDim + yDim + zDim) + 1;
                    ...
                    parallel_for(MDRangePolicy<Rank<3>>({0, 0, 0}, {2 * zDim + 1, 2 * yDim + 1, 2 * xDim + 1}),
                ...
                case 2: {
                    switch (type) {
                        case ElementType::Quad4: {
                            ...
                            parallel_for(MDRangePolicy<Rank<2>>({0, 0}, {yDim + 1, xDim + 1}),
                ...
                case 3: {
                    switch (type) {
                        case ElementType::Hex8: {
                            ...
                            parallel_for(MDRangePolicy<Rank<3>>({0, 0, 0}, {zDim + 1, yDim + 1, xDim + 1}),
        ...
        struct AddElem {
            ...
            AddElem(ViewMatrixType<Integer> el, ViewVectorType<bool> ac, Integer xdm)
            ...
            AddElem(ViewMatrixType<Integer> el,
                    ViewVectorType<bool> ac,
                    ViewMatrixTexture<Integer, hex_n_nodes> hxs,
                    ViewMatrixTextureC<unsigned int, hex_n_sides, hex_side_n_nodes> map,
                    Integer xdm,
                    Integer ydm,
                    Integer zdm)
            ...
            void operator()(int index) const {
                switch (ManifoldDim_) {
                    ...
                    case 3: {
                        ...
                        elem(index, 1) =
                            hexes(cube_index, map_side_nodes(i, 8)) / 2;  // mid-face point always the last element.
                        elem(index, 2) = (                                // rotation to catch all combinations.
                                             k == 3 ? hexes(cube_index, map_side_nodes(i, 0))
                                                    : hexes(cube_index, map_side_nodes(i, k + 1))) /
                                         2;
            ...
            // alternative approach for the 2D generation with a stride access of 2 as opposed to the first totally
            // coalesced case.
            /*
             KOKKOS_INLINE_FUNCTION
             void operator()(int i, int j) const {
        ...
        inline bool generate_elements(const int xDim, const int yDim, const int zDim) {
            using namespace Kokkos;
            ...
            switch (ManifoldDim_) {
                ...
                case 3: {
                    ...
                    parallel_for(MDRangePolicy<Rank<3>>({0, 0, 0}, {zDim, yDim, xDim}), el);
        ...
        struct AddNonSimplexElem {
            ...
            AddNonSimplexElem(ViewMatrixType<Integer> el, ViewVectorType<bool> ac, Integer xdm)
                : elem(el), active(ac), xDim(xdm) {}

            AddNonSimplexElem(ViewMatrixType<Integer> el, ViewVectorType<bool> ac, Integer xdm, Integer ydm)
                : elem(el), active(ac), xDim(xdm), yDim(ydm) {}
        ...
        inline bool generate_elements(const int xDim, const int yDim, const int zDim, Integer type) {
            using namespace Kokkos;
        ...
        struct RefineMesh {
            ...
            void init(const Mesh &m) {
                ...
                Kokkos::parallel_for(
                    "CreateMeshObject", 1, KOKKOS_LAMBDA(const int &) { new ((Mesh *)tmp) Mesh(m); });

                mesh = tmp;  // otherwise "this" pointer is still a host pointer within the parallel_for.
            }

            RefineMesh(const Mesh &m, Integer xdm) : xDim(xdm) { init(m); }
            /*~RefineMesh() {
        ...
        inline bool refine_mesh(const int xDim) {
            ...
            parallel_for(
                "DestroyMeshObject", 1, KOKKOS_LAMBDA(const int &) { tmp->~Mesh(); });

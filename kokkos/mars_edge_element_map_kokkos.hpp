...
namespace mars {
    ...
    public:
        ...
        /*
                        ...
                        still be present in the list then it will be 2* 13= 26 which generalizes in power(manifoldDim,
           manifoldDim). This + ManifoldDim grace allocation is also done for special cases or bad quality meshes (very
           small and very large angles).
        */
        ...
        MARS_INLINE_FUNCTION static void update_elem(
            const UnorderedMap<Side<N, KokkosImplementation>, ElementVector> &mapping,
            const Elem &e,
            ViewMatrixTextureC<Integer, Comb::value, N> combs) {
            TempArray<Integer, N> nodes;
            for (Integer i = 0; i < Comb::value; ++i) {
        ...
        struct UMapUpdate {
            ...
            /*void init(const PMesh *m)
            {
                    ...
                    {
                            new ((PMesh*)tmp) PMesh(*m); //same as this.mesh if mesh instead of tmp and this is a host
            pointer.
                    });
            ...
            UMapUpdate(UnorderedMap<Side<N, KokkosImplementation>, ElementVector> mp,
                       PMesh *ms,
                       ViewVectorType<Integer> ae,
                       ViewMatrixTextureC<Integer, Comb::value, N> cmb)
        ...
        template <class Mesh_>
        void update(Mesh_ *mesh, const ViewVectorType<Integer> active_elems) {
        ...
        struct UMapDescribe {
            ...
            UMapDescribe(UnorderedMap<Side<N, KokkosImplementation>, ElementVector> m) : mapping(m) {}

            MARS_INLINE_FUNCTION
            void operator()(int element_id) const {
                int count = 0;
                printf("mapping.size(): %i ", mapping.capacity());
                for (int element_id = 0; element_id < mapping.capacity(); ++element_id) {
        ...
        Integer capacity() { return mapping_.capacity(); }

        Integer size() { return mapping_.size(); }

// Throughput measurement for the EMITTER-GENERATED batched HO operator
// (warp_batched_sm90.ptx): one warp per element, grid = E blocks. Spot-checks
...
int main(int argc, char** argv)
{
    ...
    *(void**)&p_cuGetErrorString = sy(lib,"cuGetErrorString");
    ...
    const char* ptxPath = argc > 1 ? argv[1] : "../generated/warp_batched_sm90.ptx";
    ...
    CUfunction fn; CK(p_cuModuleGetFunction(&fn, mod, "full_batched"));
    ...
    for(int d=0;d<3;++d){
        ...
        for(int l=0;l<P;++l){const double* ip=&ia[l*nn];const double* dv=&da[l*nn];
            ...
            for(int i=0;i<nn;++i)flux[i]=g2[l*nn+i]*dv[i]+g0[l*nn+i]*dt2[i]+g1[l*nn+i]*dt1[i];
    ...
    size_t elemB = nnn*sizeof(double);
    auto allocBatched = [&](const double* elem0)->CUdeviceptr {
        CUdeviceptr d; CK(p_cuMemAlloc(&d, (size_t)E*elemB));
        CK(p_cuMemcpyHtoD(d, elem0, elemB));                 // element 0
        for (long e=1;e<E;++e) CK(p_cuMemcpyDtoD(d+(size_t)e*elemB, d, elemB));
        return d;
    };
    CUdeviceptr dU = allocBatched(u.data());
    CUdeviceptr dG[9]; for(int i=0;i<9;++i) dG[i]=allocBatched(g[i].data());
    ...
    for(int i=0;i<9;++i) add4(&dG[i]); add4(&dY);
    ...
    printf("batched operator: E=%ld  spot-check(elem0) max|y-Knaus| = %.3e  %s\n",
    ...
    double bytes = (double)E*(1+9+1)*elemB;              // u + 9 metric + y

// GPU gate for the EMITTER-GENERATED FULL operator, transpose INTERNALIZED
// (warp_fulls_sm90.ptx): SINGLE u (8x8x8) input -- each direction reads u
// planes via emit_bsweep (pax/arm absorb the moveaxis). 127 params.: all three directions (each B-sweep + 7 faces + plane
...
int main(int argc, char** argv)
{
    ...
    *(void**)&p_cuGetErrorString = must_sym(lib, "cuGetErrorString");
    ...
    const char* ptxPath = argc > 1 ? argv[1] : "../generated/warp_fulls_sm90.ptx";
    ...
    dev_ptrs.push_back(alloc(u.data(), nnn));   // single 8x8x8 field
    ...
    add3(&dev_ptrs[0]);                       // u (8x8x8)
    for (int i = 1; i < 5; ++i) add2(&dev_ptrs[i], sz2, st2);  // Btil,Dtil,Dm,W
    for (int i = 5; i < 14; ++i) add3(&dev_ptrs[i]);           // 9 metric
    add3(&dev_ptrs[14]);                      // y
    ...
    printf("emitter FULL (single-u) operator gate: max|y - Knaus(u)| = %.3e  %s\n",

...
Run from mirage-mlir/:  python3 test/exec_gate.py       (needs build/tools/mir-opt)
"""
...
def gate3():
    ...
    compiler = os.path.abspath(os.path.join(ROOT, "..", "mirage-compiler"))
    emitted = subprocess.run(
        [sys.executable, "mirage-emit.py", "specs/laplacian.op", "--backend", "mlir"],
        cwd=compiler, capture_output=True, text=True)
    if emitted.returncode:
        sys.exit(f"mirage-emit failed:\n{emitted.stderr}")

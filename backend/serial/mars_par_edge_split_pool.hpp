...
namespace mars {
    ...
    public:
        ...
        void build_edge_interface(ParMesh &p_mesh, Bisection<typename ParMesh::Mesh, EdgeSelect> &bisection) {
            ...
            for (Integer i = 0; i < mesh.n_elements(); ++i) {
                ...
                for (Integer i = 0; i < n_edges(e); ++i) {
                    ...
                    Edge global_edge(p_mesh.node_map().global(edge.nodes[0]), p_mesh.node_map().global(edge.nodes[1]));
                    ...
                    p_mesh.node_map().intersect_partitions(std::begin(edge.nodes), std::end(edge.nodes), sharing);
        ...
        template <Integer Dim, Integer ManifoldDim>
        void update_midpoint_parts(const EdgeNodeMap &edge_node_map, ParMesh<Dim, ManifoldDim> &mesh) {}
        ...
        template <Integer Dim, Integer ManifoldDim>
        void write_to_mesh(const EdgeNodeMap &edge_node_map, ParMesh<Dim, ManifoldDim> &mesh) {}

        template <Integer Dim, Integer ManifoldDim>
        void read_from_mesh(const EdgeNodeMap &edge_node_map, const ParMesh<Dim, ManifoldDim> &mesh) {}

        template <Integer Dim, Integer ManifoldDim>
        void collect_splits_to_local_edges(const ParMesh<Dim, ManifoldDim> &mesh, std::vector<Edge> &local_splits) {
            local_splits.clear();
        ...
        bool empty() const { return comm_.reduce_and(splits_.empty() && to_communicate_.empty()); }
        ...
        template <class ParMesh>
        void collect_splits_to_apply(const ParMesh &part, std::vector<EdgeSplit> &local_splits) {
            assert(edge_to_split_map_.size() == splits_.size());

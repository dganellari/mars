...
namespace mars {
    ...
    public:
        ...
        PrecomputeLeppBisection(Mesh &mesh) : Bisection<Mesh>(mesh) {
        ...
        void refine_element(const Integer element_id) {
            ...
            while (Bisection<Mesh>::get_mesh().is_active(element_id)) {
                ...
                precompute_lepp_incidents(
                    Bisection<Mesh>::get_mesh());  // TODO: heavier part. Should be done only once in parallel. after
                                                   // each update of the mesh from all threads.
    ...
    private:
        ...
        void compute_lepp(const Integer element_id) {
            /*	Integer edge_num = Bisection<Mesh>::edge_select()->stable_select(Bisection<Mesh>::get_mesh(),
               element_id);
                    //Integer edge_num = Bisection<Mesh>::edge_select()->select(Bisection<Mesh>::get_mesh(),
               element_id);
                    //Integer edge_num = Bisection<Mesh>::edge_select()->select(Bisection<Mesh>::get_mesh(), element_id,
               Bisection<Mesh>::edge_element_map());
...
Bisection<Mesh>::edge_select()->stable_select(Bisection<Mesh>::get_mesh(), i); const Integer edge_num =
Bisection<Mesh>::edge_select()->select(Bisection<Mesh>::get_mesh(), edge, i);
    ...
    public:
        ...
        PrecomputeLeppTreeBisection(Mesh &mesh) : Bisection<Mesh>(mesh) {
    ...
    private:
        ...
        not updated. Leads to unconforming mesh and inf. loop.

...
namespace mars {
    ...
    public:
        ...
        explicit EdgeSplitPool(const Integer partition_id, const Integer n_parts)
        ...
        void pack(std::vector<std::vector<EdgeSplit>> &splits, const bool clear_buffers) {
        ...
        inline void unpack(const Integer originator, const std::vector<EdgeSplit> &splits) {
            for (const auto &s : splits) {
                add_global_split(originator, s);
            }
        }

        const EdgeSplit &add_global_split(const Integer originator, const EdgeSplit &edge_split) {
            assert(!edge_split.partitions.empty());
        ...
        inline void resolve_midpoint_id(const Edge &e, const Integer local_m_id, const Integer m_id, Map &node_map) {
            auto &s = get_split(e);
            assert(s.is_valid());
        ...
        template <class Mesh>
        void set_midpoint_ids_to(const EdgeNodeMap &enm, MeshPartition<Mesh> &part) {
        ...
        template <class Mesh>
        void collect_splits_to_apply(const MeshPartition<Mesh> &part, std::vector<EdgeSplit> &local_splits) {
            assert(edge_to_split_map.size() == splits_.size());
        ...
        template <class Mesh>
        void collect_splits_to_local_edges(const MeshPartition<Mesh> &part, std::vector<Edge> &local_splits) {
            local_splits.clear();
            local_splits.reserve(splits_.size());
        ...
        inline bool empty() const { return splits_.empty() && to_communicate.empty(); }
        ...
        inline std::vector<EdgeSplit>::const_iterator begin() const { return splits_.begin(); }

        inline std::vector<EdgeSplit>::const_iterator end() const { return splits_.end(); }
        ...
        template <class Mesh>
        void build_edge_interface(const std::vector<ptr<Bisection<Mesh, EdgeSelect<Mesh>>>> &b,
            ...
            for (Integer i = 0; i < mesh.n_elements(); ++i) {
                ...
                for (Integer i = 0; i < n_edges(e); ++i) {
                    ...
                    Edge global_edge(p.node_map().global(edge.nodes[0]), p.node_map().global(edge.nodes[1]));
                    ...
                    p.node_map().intersect_partitions(std::begin(edge.nodes), std::end(edge.nodes), sharing);
        ...
        template <class Mesh>
        void update_midpoint_parts(const EdgeNodeMap &enm, MeshPartition<Mesh> &p) {
            for (const auto &ei : edge_interface_) {
        ...
        void update_edge_interface(const Edge &global_edge, const Integer global_midpoint) {

...
namespace mars {
    ...
    public:
        ...
        inline void set_verbose(const bool val) { verbose_ = val; }
        ...
        inline bool has_pending_requests() const { return !recv_reqs_.empty() || !send_reqs_.empty(); }
        ...
        ///@brief returns the number of incoming recv (use either wait_all_recv or i_probe to ensure recvs are
        /// completed)
        template <class OutputBuffer, class InputBuffer>
        Integer i_send_recv_all(std::vector<OutputBuffer> &send_buffer, std::vector<InputBuffer> &recv_buffer) {
        ...
        struct Activity {
            ...
            inline bool empty() const { return rank < 0; }
        ...
        inline bool is_valid(const Integer rank) { return (rank >= 0) && (rank < size()); }

        static bool is_null(Integer rank);

        static inline bool probe_null(const Integer count) { return count == -1; }

        void describe(std::ostream &os) const { os << "[" << rank() << "] "; }
        ...
        // bool unstructured_all_gather(ByteOutputStream &send_buffer, std::vector<ByteInputBuffer> &recv_buffer, const
        // bool blocking);
    };

    template <class Fun>
    void serial_apply(const Communicator &comm, Fun fun) {
    ...
    template <class Fun>
    void root_apply(const Communicator &comm, Fun fun) {
        comm.barrier();

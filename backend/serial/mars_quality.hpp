...
namespace mars {
    ...
    public:
        ...
        class Utils {
        public:
            static Real surface_area(const Simplex<Dim, ManifoldDim> &s, const std::vector<Vector<Real, Dim>> &points) {
            ...
            static Real avg_edge_len(const Simplex<Dim, ManifoldDim> &s, const std::vector<Vector<Real, Dim>> &points) {
                Integer v1, v2;
                Real ret = 0.;
            ...
            static Real rms_edge_len(const Simplex<Dim, ManifoldDim> &s, const std::vector<Vector<Real, Dim>> &points) {
                Integer v1, v2;
                Real ret = 0.;
            ...
            static Real max_edge_len(const Simplex<Dim, ManifoldDim> &s, const std::vector<Vector<Real, Dim>> &points) {
                Integer v1, v2;
            ...
            static Real min_edge_len(const Simplex<Dim, ManifoldDim> &s, const std::vector<Vector<Real, Dim>> &points) {
                Integer v1, v2;
                Real ret = std::numeric_limits<Real>::max();
            ...
            static Real max_side_area(const Simplex<Dim, ManifoldDim> &s,
            ...
            static Real min_side_area(const Simplex<Dim, ManifoldDim> &s,
                                      const std::vector<Vector<Real, Dim>> &points) {
                Simplex<Dim, ManifoldDim - 1> side;
            ...
            static Real avg_side_area(const Simplex<Dim, ManifoldDim> &s,
                                      const std::vector<Vector<Real, Dim>> &points) {
        ...
        public:
            ...
            virtual void init(const Mesh &m) { q_.resize(m.n_elements()); }

            virtual void compute_and_store(const Simplex<Dim, ManifoldDim> &s,
                                           const std::vector<Vector<Real, Dim>> &points) {
                q_[s.id] = compute(s, points);
            }

            virtual Real compute(const Simplex<Dim, ManifoldDim> &s, const std::vector<Vector<Real, Dim>> &points) = 0;
        ...
        public:
            Real compute(const Simplex<Dim, ManifoldDim> &s, const std::vector<Vector<Real, Dim>> &points) override {
        ...
        public:
            ...
            std::string name() const override { return "Tau"; }
        ...
        public:
            ...
            void add_data_point(const std::string &name, const Integer n_elements, const Real value) {
                auto &x_ptr = x[name];
                auto &y_ptr = y[name];
            ...
            bool save_csv(const std::string &name, const std::string &path, const bool print_header = true) const {
                std::ofstream os;
                os.open(path);
                if (!os.good()) {
                    return false;
        ...
        Quality(const Mesh &mesh) : mesh(mesh) { init(); }
        ...
        void compute() {
            ...
            for (auto m : measures) {
                report.add_data_point(m->name() + "max", na, m->q_max);

                report.add_data_point(m->name() + "min", na, m->q_min);

                report.add_data_point(m->name() + "avg", na, m->q_avg);
            }
        }
        ...
        bool save_report(const std::string &path) { return report.save(path); }

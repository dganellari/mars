...
namespace mars {
            ...
            for (Integer j = 0; j <= 2 * yDim; ++j) {
                for (Integer i = 0; i <= 2 * xDim; ++i) {
                    Vector<Real, Dim> p({
                        ...
                        static_cast<Real>(j) / static_cast<Real>(2 * yDim),
                        static_cast<Real>(k) / static_cast<Real>(2 * zDim),
                    });
                    ...
                    if (k == 0) add_side(sides(0), i, j, ind);

                    if (k == 2) add_side(sides(1), i, j, ind);

                    // if (k == 2 * (zDim - 1))
                    if (j == 0) add_side(sides(2), i, k, ind);

                    // if (j == 2 * (yDim - 1))
                    if (j == 2) add_side(sides(3), i, k, ind);

                    if (i == 0) add_side(sides(4), j, k, ind);

                    // if (i == 2 * (xDim - 1))
                    if (i == 2) add_side(sides(5), j, k, ind);
                }
            }
        }
    ...
    bool generate_cube(Mesh<Dim, ManifoldDim>& mesh, const Integer xDim, const Integer yDim, const Integer zDim) {
        ...
        switch (ManifoldDim) {
            ...
            case 3: {  // building the tetra mesh from the hex27 elem
                ...
                const int n_tetra_nodes =
                    5 * (xDim * yDim * zDim) + 2 * (xDim * yDim + xDim * zDim + yDim * zDim) + (xDim + yDim + zDim) + 1;
                ...
                // First the center node is added somewhere at the end of the vector and then all the other nodes are
                // added to it.
                        ...
                        for (Integer i = 0; i < 2 * xDim; i += 2) {
                            ...
                            for (unsigned int i = 0; i < hex_n_sides; ++i) {
                                ...
                                for (unsigned int k = 0; k < 4; k++) {
                                    ...
                                    e.nodes[2] =
                                        (k == 3 ? side[0] : side[k + 1]) / 2;  // rotation to catch all combinations.
                                    e.nodes[3] = centerHex;                    // the center of the cube.

                                    active_nodes[side[k]] = true;
                                    active_nodes[side[8]] = true;
                                    active_nodes[(k == 3 ? side[0] : side[k + 1])] = true;
                ...
                for (Integer k = 0; k <= 2 * zDim; ++k) {
                    for (Integer j = 0; j <= 2 * yDim; ++j) {
                        for (Integer i = 0; i <= 2 * xDim; ++i) {
                            ...
                            if (active_nodes[in]) {
                                mesh.point(in / 2)[0] = static_cast<Real>(i) / static_cast<Real>(2 * xDim);
                                mesh.point(in / 2)[1] = static_cast<Real>(j) / static_cast<Real>(2 * yDim);
                                mesh.point(in / 2)[2] = static_cast<Real>(k) / static_cast<Real>(2 * zDim);
                            }
                        }
                    }
                }
    ...
    inline bool generate_line(Mesh1& mesh, const Integer xDim) { return generate_cube(mesh, xDim, 0, 0); }
    ...
    bool generate_cube(Mesh<Dim, ManifoldDim, DefaultImplementation, NonSimplex<Type>>& mesh,
                       const Integer xDim,
                       const Integer yDim,
                       const Integer zDim) {
        ...
        switch (ManifoldDim) {
            case 2: {
                switch (Type) {
                    ...
                    default: {
                        std::cerr << "Not yet implemented for other 2D element types." << std::endl;
                        return false;
                    }
                }
            }
            case 3: {
                switch (Type) {
                    ...
                    default: {
                        std::cerr << "Not yet implemented for other 3D element types." << std::endl;
                        return false;
                    }
                }
            }

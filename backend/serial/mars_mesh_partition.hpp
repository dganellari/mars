...
namespace mars {
    ...
    public:
        ...
        MeshPartition(const Integer partition_id, const Integer n_partitions)
            : node_map_(partition_id, n_partitions), elem_map_(partition_id, n_partitions) {}

        Integer partition_id() const { return node_map_.partition_id(); }

        void mark_partition_boundary(const Integer local_element_id,
        ...
        void add_and_index_nodes(const Mesh &parent_mesh,
            ...
            for (Integer i = 0; i < mesh.n_elements(); ++i) {
                ...
                for (Integer k = 0; k < parent_e.nodes.size(); ++k) {
                    ...
                    if (visited[n] == INVALID_INDEX) {
                        ...
                        node_map_.add(local_n, n, node_partitioning[n]);
        ...
        void update_edge_split_pool(const EdgeElementMap &eem,
                                    const EdgeNodeMap &enm,
                                    const std::vector<Edge> &local_edges,
                                    EdgeSplitPool &pool) {
            for (auto &e : local_edges) {
        ...
        void read_from_edge_pool(const EdgeNodeMap &enm, EdgeSplitPool &pool) { pool.set_midpoint_ids_to(enm, *this); }

        void write_to_edge_pool(const EdgeNodeMap &enm, EdgeSplitPool &pool) {
            for (auto it = pool.begin(); it != pool.end(); ++it) {
                ...
                pool.resolve_midpoint_id(e_split.edge, midpoint, node_map_.global(midpoint), node_map_);
            }
        }

        Integer local_midpoint(const EdgeNodeMap &enm, const Edge &global_edge) const {
        ...
        Integer global_midpoint(const EdgeNodeMap &enm, const Edge &global_edge) const {
        ...
        void update_midpoint_ids(const EdgeNodeMap &enm, const std::vector<EdgeSplit> &global_edges) {
        ...
        void assign_node_local_to_global(const Integer local_id, const Integer global_id) {
            node_map_.insert_local_to_global(local_id, global_id);
        }

        void localize_edges(const std::vector<Edge> &global_edges, std::vector<Edge> &local_edges) {
            local_edges.clear();
        ...
        inline Integer max_global_node_id() const { return node_map_.max_id(); }

        inline Integer max_global_elem_id() const { return elem_map_.max_id(); }

        Integer n_owned_nodes() const { return node_map_.n_owned(); }

        void assign_node_owner(const Integer local_node_id, const Integer owner) {
            node_map_.set_owner(local_node_id, owner);
        }

        void assign_global_node_ids(const Integer begin, const Integer end) { node_map_.assign_global_ids(begin, end); }

        void assign_global_elem_ids(const Integer begin, const Integer end) { elem_map_.assign_global_ids(begin, end); }

        const Integer global_node_id(const Integer local_node_id) const { return node_map_.global(local_node_id); }

        inline const Map &node_map() const { return node_map_; }

        inline const Map &elem_map() const { return elem_map_; }

        inline Map &node_map() { return node_map_; }

        inline Map &elem_map() { return elem_map_; }
    ...
    private:
        static bool is_partition_tag(const Integer tag) { return tag < INVALID_INDEX; }

        static Integer partition_id_to_tag(const Integer adj_partition) { return -(adj_partition + 2); }

        static Integer tag_to_partition_id(const Integer tag) { return -(tag + 2); }

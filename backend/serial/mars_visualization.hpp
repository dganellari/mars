...
namespace mars {
    ...
    public:
        ...
        PlotOpts()
            : plot_fun(PLOT_ID), scale_factor(10.), node_size(2.), uniform(0.), active_only(true), show_id(true) {}
    };
    ...
    template <class Mesh>
    bool export_parts(const std::string &path, const std::vector<std::shared_ptr<Mesh>> &parts) {
        bool ok = true;
    ...
    void draw_element_side(const Mesh<Dim, ManifoldDim> &m,
        ...
        draw_element_subsurface(m, element_id, side.nodes, cx, cy, scale_factor, rgb, canvas, sort_nodes);
    }
    ...
    void draw_element_as_child(const Mesh<Dim, ManifoldDim> &m,
        ...
        for (Integer k = 0; k < ManifoldDim + 1; ++k) {
            ...
            for (Integer j = k + 1; j < ManifoldDim + 1; ++j) {
                ...
                if (it != c_nodes.end()) {
                    ...
                    c_xy[index * 2] = cx + std::cos(dAngleMid / 2 + dAngleMid * midpoint_index<ManifoldDim>(k, j)) *
                                               scale_factor / 1.5;
                    c_xy[index * 2 + 1] = cy + std::sin(dAngleMid / 2 + dAngleMid * midpoint_index<ManifoldDim>(k, j)) *
                                                   scale_factor / 1.5;
                    c_index[index] = v;
        ...
        for (Integer k = 0; k < n_nodes(c); ++k) {
            if (!is_midpoint[k]) continue;
            canvas.draw_text(c_xy[k * 2],
                             c_xy[k * 2 + 1],
                             0.5,
                             "Courier",
                             std::to_string(c_index[k]) + "=(" + std::to_string(edge[k].nodes[0]) + "," +
                                 std::to_string(edge[k].nodes[1]) + ")",
                             true);
        }
    ...
    bool write_element(const std::string &path,
        ...
        if (!e.children.empty()) {
            if (child_num != INVALID_INDEX) {
                ...
                draw_element_as_child(
                    m, en_map, c, cx, cy, scale_factor, RGB(hsv[c * 3], hsv[c * 3 + 1], hsv[c * 3 + 2]), canvas);
        ...
        draw_element(m, element_id, cx, cy, scale_factor, e_rgb, canvas);
        ...
        for (Integer i = 0; i < adj.size(); ++i) {
            ...
            draw_element(m, adj[i], a_cx, a_cy, scale_factor, rgb, canvas);

            for (auto e_c : neigs) {
                if (m.is_child(adj[i], e_c)) {
                    draw_element_as_child(m,
                                          en_map,
                                          e_c,
                                          a_cx,
                                          a_cy,
    ...
    bool write_element(const std::string &path,
                       ...
                       const Real scale_factor,
                       const Integer child_num) {
        return write_element(path, rgr.get_mesh(), rgr.edge_node_map(), element_id, scale_factor, child_num);
    }
        ...
        for (std::size_t i = 0, k = 0; i < mesh.n_elements(); ++i) {
            if (mesh.is_active(i)) {
                for (auto n : mesh.elem(i).nodes) {
                    ...
                    canvas.update_box(p(0) * opts.scale_factor + opts.scale_factor / 10.,
                                      p(1) * opts.scale_factor + opts.scale_factor / 10.);
                    canvas.update_box(p(0) * opts.scale_factor - opts.scale_factor / 10.,
                                      p(1) * opts.scale_factor - opts.scale_factor / 10.);
                }
            }
        ...
        if (opts.show_id) {
            ...
            for (std::size_t i = 0, k = 0; i < mesh.n_elements(); ++i) {
                if (mesh.is_active(i)) {
                    ...
                    canvas.draw_text(b(0) * opts.scale_factor,
                                     b(1) * opts.scale_factor,
                                     opts.node_size,
                                     "Courier",
                                     std::to_string(e_id),
                                     true);
                    ...
                    for (auto n : mesh.elem(i).nodes) {
                        ...
                        if (n_id != INVALID_INDEX) {
                            canvas.draw_text(p(0) * opts.scale_factor,
                                             p(1) * opts.scale_factor,
                                             opts.node_size,
                                             "Courier",
                                             std::to_string(n_id),
                                             true);
    ...
    void draw_element_subsurface(const Mesh<Dim, ManifoldDim> &m,
        ...
        Integer index = 0;
        for (Integer k = 0; k < n_nodes(e); ++k) {
            if (std::find(sub_surface_nodes.begin(), sub_surface_nodes.end(), e.nodes[k]) != sub_surface_nodes.end()) {
    ...
    bool write_element_with_sides(const std::string &path,
        ...
        if (side_num != INVALID_INDEX) {
            ...
            if (!skip) {
                ...
                draw_element_side(m, element_id, side_num, cx, cy, scale_factor, rgb, canvas, false);
                ...
                if (a != INVALID_INDEX) {
                    ...
                    draw_element(m, a, a_cx, a_cy, scale_factor, rgb, canvas, false);

                    draw_element_side(
                        m, a, m.common_side_num(a, element_id), a_cx, a_cy, scale_factor, e_rgb, canvas, false);
                }
            }
            ...
            for (Integer s = 0; s < n_sides(e); ++s) {
                ...
                if (!skip) {
                    ...
                    draw_element_side(m, element_id, s, cx, cy, scale_factor, rgb, canvas, false);
        ...
        draw_element(m, element_id, cx, cy, scale_factor, e_rgb, canvas, false);

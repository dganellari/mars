...
namespace mars {
    ...
    public:
        ...
        Bisection(Mesh &mesh, std::shared_ptr<EdgeSelect_> edge_select = std::make_shared<EdgeSelect_>())
            : mesh(mesh), edge_select_(edge_select), verbose(false), fail_if_not_refine(false) {}

        void set_fail_if_not_refine(const bool val) { fail_if_not_refine = val; }

        bool get_fail_if_not_refine() { return fail_if_not_refine; }

        std::vector<Integer> &get_incomplete_elements() { return incomplete_elements_; }

        virtual void set_edge_select(const std::shared_ptr<EdgeSelect_> &edge_select) { edge_select_ = edge_select; }

        std::shared_ptr<EdgeSelect_> edge_select() { return edge_select_; }
        ...
        void other_nodes(const std::array<Integer, ManifoldDim + 1> &nodes,
                         const Integer v1,
                         const Integer v2,
        ...
        void bisect_element(const Integer element_id, const Edge &edge) {
        ...
        void bisect_element(const Integer element_id, const Integer v1, const Integer v2) {
            mesh.elem(element_id).children.clear();
            mesh.set_active(element_id, false);
            ...
            edge_select_->element_refined(mesh, element_id, Edge(v1, v2), midpoint);

            return;
        }

        inline Integer side_num(const Integer element_id, const SideElem &side) const {
        ...
        void bisect_side_tags(const Integer element_id, const Edge &edge, const Integer midpoint_id) {
            const auto &e = mesh.elem(element_id);

            for (auto c : e.children) {
                std::fill(mesh.elem(c).side_tags.begin(), mesh.elem(c).side_tags.end(), INVALID_INDEX);
            }
            ...
            for (Integer i = 0; i < n_sides(e); ++i) {
                ...
                if (has_edge(side, edge[0], edge[1])) {
                    ...
                    for (auto c : e.children) {
                        ...
                        if (INVALID_INDEX != sn) {
                            assert(mesh.elem(c).side_tags[sn] == INVALID_INDEX || mesh.elem(c).side_tags[sn] == tag);
        ...
        bool refine_element_recursive(const Integer element_id, const Edge &edge, const Integer max_level) {
        ...
        const EdgeElementMap &edge_element_map() const { return edge_element_map_; }

        EdgeElementMap &edge_element_map() { return edge_element_map_; }
        ...
        const std::vector<Edge> &bisected_edges() const { return bisected_edges_; }

        void clear_bisected_edges() { bisected_edges_.clear(); }
        ...
        void tracking_begin() { tracker_.begin_iterate(); }

        void tracking_end() { tracker_.end_iterate(); }

        void undo() { tracker_.undo_last_iterate(mesh); }

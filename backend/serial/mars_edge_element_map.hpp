...
namespace mars {
    ...
    public:
        ...
        const ElementVector &elements(const Side<N> &side) const { return map_.elements(side); }
        ...
        template <Integer Dim, Integer ManifoldDim>
        void adj(const Integer sub_manifold_dim, const Simplex<Dim, ManifoldDim> &e, std::vector<Integer> &res) const {
    ...
    public:
        ...
        const ElementVector &elements(const std::vector<Integer> &side) const { return elements(Side<N>(side)); }

        void describe(std::ostream &os) const { map_.describe(os); }

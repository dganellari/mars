...
namespace mars {
    ...
    public:
        inline void fun(const Vector<Real, Dim> &x_ref, std::vector<Real> &values) const {
        ...
        inline void grad(const Vector<Real, Dim> &x_ref, std::vector<Vector<Real, Dim>> &values) const {
            for (Integer i = 0; i < Dim; ++i) {
                values[0](i) = -1;

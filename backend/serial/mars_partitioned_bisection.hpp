    ...
    public:
        ...
        PartitionedBisection(std::vector<ptr<P>> &parts, std::shared_ptr<ES> edge_select = std::make_shared<ES>())
            ...
            for (auto &m : parts) {
                ...
                edge_split_pool_.push_back(std::make_shared<EdgeSplitPool>(m->partition_id(), parts.size()));
            }
        }
        ...
        void build_edge_interface() {
            ...
            for (auto &m : parts) {
                edge_split_pool_[m->partition_id()]->build_edge_interface(bisection, parts);
        ...
        void update_edge_pools() {
            // parallel for
            for (Integer k = 0; k < parts.size(); ++k) {
                parts[k]->update_edge_split_pool(bisection[k]->edge_element_map(),
        ...
        void update_global_ids() {
            // parallel for
            for (Integer k = 0; k < parts.size(); ++k) {
                parts[k]->read_from_edge_pool(bisection[k]->edge_node_map(), *edge_split_pool_[k]);
            ...
            // parallel for
            for (Integer k = 0; k < parts.size(); ++k) {
                parts[k]->write_to_edge_pool(bisection[k]->edge_node_map(), *edge_split_pool_[k]);
            }
        ...
        bool conform_interfaces() {
            ...
            for (Integer k = 0; k < parts.size(); ++k) {
                ...
                edge_split_pool_[k]->collect_splits_to_local_edges(*parts[k], splits);
        ...
        void print_analysis(const Integer partition_id) {
                ...
                if (!invalid_nodes.empty()) {
                    ...
                    for (Integer k = 0; k < n_edges(parent); ++k) {
                        ...
                        for (auto in : invalid_nodes) {
                            ...
                            Edge global_edge(part.node_map().global(edge.nodes[0]),
                                             part.node_map().global(edge.nodes[1]));

                            std::cout << midpoint << " split(" << global_edge.nodes[0] << "," << global_edge.nodes[1]
                                      << ")\n";
                    ...
                    if (incident.empty()) {
                        ...
                        if (!incident.empty()) {
                            ...
                            mesh.find_elements_by_nodes(
                                local_edge.nodes.begin(), local_edge.nodes.end(), elems, false, true);
            ...
            for (auto es : inconsistent_edge_splits) {
                ...
                es.describe(std::cout);
                std::cout << "found midpoint in part " << owner << " with lid " << lmp << " and gid " << mp
                          << std::endl;
                ...
                edge_split_pool_[partition_id]->edge_interface(es.edge, edge_interface);
...
#endif  // MARS_PARTITIONED_BISECTION_HPP

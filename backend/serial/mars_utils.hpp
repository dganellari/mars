...
namespace mars {
    ...
    void mark_hypersphere_for_refinement(const Mesh<Dim, ManifoldDim> &mesh,
    ...
    template <Integer Dim, Integer ManifoldDim>
    void export_mesh(const Mesh<Dim, ManifoldDim> &mesh, std::ostream &os) {
        os << "MFEM mesh v1.0\n\n";
        os << "dimension\n" << ManifoldDim << "\n";
    ...
    template <Integer Dim, Integer ManifoldDim>
    void export_elems_with_bad_tags(const Mesh<Dim, ManifoldDim> &mesh,
                                    std::ostream &os,
    ...
    template <Integer Dim, Integer ManifoldDim>
    void print_boundary_tags(const Simplex<Dim, ManifoldDim> &e) {
    ...
    template <Integer Dim, Integer ManifoldDim>
    void print_boundary_points(const Mesh<Dim, ManifoldDim> &mesh,
                    ...
                    if (not_on_unit_cube) {
                        for (auto n : side.nodes) {
                            ...
                            for (Integer j = 0; j < Dim; ++j) {
                                if (std::abs(p[j] - 1.) < 1e-12 || std::abs(p[j]) < 1e-12) {
        ...
        for (Integer i = 0; i < mesh.n_elements(); ++i) {
            ...
            for (Integer f = 0; f < adj.size(); ++f) {
                ...
                if (adj_partition_id != partition_id) {
                    p->mark_partition_boundary(local_element_id, f, adj_partition_id);
                }
            }
    ...
    bool write_mesh(const std::string &path, const Mesh<Dim, 4> &mesh) {
    ...
    template <Integer Dim>
    bool write_mesh(const std::string &path, const Mesh<Dim, 5> &mesh) {
        std::ofstream os(path + ".MFEM");
        if (!os.good()) return false;
        export_mesh(mesh, os);
        os.close();
    ...
    template <Integer Dim>
    bool write_mesh(const std::string &path, const Mesh<Dim, 6> &mesh) {
        std::ofstream os(path + ".MFEM");
        if (!os.good()) return false;
        export_mesh(mesh, os);
        os.close();

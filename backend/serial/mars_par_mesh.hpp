...
namespace mars {
    ...
    public:
        ...
        inline Integer n_active_elements() const { return n_active_elements_; }

        inline Integer n_nodes() const { return n_nodes_; }

        inline Integer n_local_active_elements() const { return mesh_.n_active_elements(); }

        inline Integer n_local_elements() const { return mesh_.n_elements(); }

        inline Integer n_local_nodes() const { return mesh_.n_nodes(); }
        ...
        ParMesh(const Communicator &comm)
            : comm_(comm),
              ...
              n_active_elements_(0),
              n_nodes_(0) {}

        void init(Mesh &serial_mesh, const std::vector<Integer> &partitioning) {
                ...
                if (partition_id == comm_.rank()) {
                    ...
                    for (Integer f = 0; f < adj.size(); ++f) {
                        ...
                        if (adj_partition_id != partition_id) {
                            this->mark_partition_boundary(local_element_id, f, adj_partition_id);
        ...
        inline bool is_interfaced(const Integer rank) const { return is_interfaced_[rank]; }

        inline Integer n_interfaced() { return n_interfaced_; }
        ...
        void synchronize() {
            ...
            std::array<Integer, 2> buff = {mesh_.n_active_elements(), node_map().n_owned()};
        ...
        void collect_interface_sides(const Integer rank,
                                     std::vector<SideElem> &sides,
                                     const bool global_indexing = true,
                                     const bool active_only = true) {
            mesh_.collect_sides(partition_id_to_tag(rank), sides, active_only);
        ...
        void exchange_interface_sides(std::vector<std::vector<SideElem>> &sides) {
            ...
            for (Integer r = 0; r < comm_.size(); ++r) {
                if (this->is_interfaced(r)) {
                    this->collect_interface_sides(r, local_sides, true, true);
        ...
        Mesh &get_serial_mesh() { return mesh_; }

        const Mesh &get_serial_mesh() const { return mesh_; }

        inline Communicator &comm() { return comm_; }
    ...
    private:
        ...
        // aux stuff
    private:
        void add_and_index_nodes(const Mesh &serial_mesh, const std::vector<Integer> &node_partitioning) {

...
// Test fixture for ElementDomain tests that creates test mesh files
...
TEST_F(ElementDomainTest, MultiRankDistribution) {
    ...
    if (rank == 0) {
        ...
        for (int r = 0; r < numRanks; r++) {
            std::cout << "Rank " << r << " has " << allElementCounts[r] << " elements and "
                      << allNodeCounts[r] << " nodes" << std::endl;
        ...
        if (nonZeroCounts.size() > 1) {
            ...
            std::cout << "Element distribution: min=" << minElems 

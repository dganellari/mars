...
void computeSfcKeysGpu(const Real* x, const Real* y, const Real* z,
                       KeyType* keys, size_t numKeys, const cstone::Box<Real>& box);
...
void ElementDomain<ElementTag, cstone::GpuTag>::computeOnElements(KernelFunc kernel) {
   ...
   // Extract raw pointers from tuple elements for kernel call
    ...
    if constexpr (std::is_same_v<ElementTag, TetTag>) {
        ...
        kernel<<<numBlocks, blockSize>>>(thrust::raw_pointer_cast(d_x.data()),
...
void ElementDomain<ElementTag, AcceleratorTag>::calculateCharacteristicSizes() {
    ...
    if constexpr (std::is_same_v<ElementTag, TetTag> && std::is_same_v<AcceleratorTag, cstone::GpuTag>) {
        ...
        Real domainDiagonal = std::sqrt(
           std::pow(domain_->box().xmax() - domain_->box().xmin(), 2) +
    ...
    if constexpr (std::is_same_v<AcceleratorTag, cstone::GpuTag>) {
        ...
        if constexpr (std::is_same_v<ElementTag, TetTag>) {
            ...
            findRepresentativeNodesKernel<TetTag>
                <<<numBlocks, blockSize>>>(thrust::raw_pointer_cast(d_i0.data()),
                                           thrust::raw_pointer_cast(d_i1.data()),
                                           thrust::raw_pointer_cast(d_i2.data()),
                                           thrust::raw_pointer_cast(d_i3.data()),
                                           thrust::raw_pointer_cast(d_nodeSfcCodes.data()),
...
void ElementDomain<ElementTag, AcceleratorTag>::sync() {
    if constexpr (std::is_same_v<AcceleratorTag, cstone::GpuTag>) {
        ...
        if constexpr (std::is_same_v<ElementTag, TetTag>) {
            findRepresentativeNodesKernel<TetTag>
                <<<numBlocks, blockSize>>>(thrust::raw_pointer_cast(getConnIndices(0).data()),
                                           thrust::raw_pointer_cast(getConnIndices(1).data()),
                                           thrust::raw_pointer_cast(getConnIndices(2).data()),
                                           thrust::raw_pointer_cast(getConnIndices(3).data()),
                                           thrust::raw_pointer_cast(d_nodeSfcCodes.data()),
                                           thrust::raw_pointer_cast(d_elemToNodeMap_.data()),
                                           elementCount_);
        } else if constexpr (std::is_same_v<ElementTag, HexTag>) {

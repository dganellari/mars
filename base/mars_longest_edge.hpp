...
namespace mars {
template <class Mesh>
class LongestEdgeSelect final : public
// FIXME: ugly as hell
#ifdef WITH_KOKKOS
                                ParallelEdgeSelect<Mesh>
#else
                                EdgeSelect<Mesh>
#endif

{
 public:
  ...
  MARS_INLINE_FUNCTION LongestEdgeSelect(const bool recursive = true,
                                         const bool use_tollerance = true)
      : recursive_(recursive), use_tollerance_(use_tollerance) {}

  MARS_INLINE_FUNCTION Integer select(const Mesh &mesh,
  ...
  Integer stable_select(const Mesh &mesh,
                        const Integer element_id) const override {
    const auto &e = mesh.elem(element_id);
  ...
  MARS_INLINE_FUNCTION
  virtual Integer select(const Mesh &mesh, const Edge &neighbor_edge,
                         const Integer element_id) const override {
    if (!use_tollerance_) {
...
class UniqueLongestEdgeSelect final : public EdgeSelect<Mesh> {
 public:
  UniqueLongestEdgeSelect(const bool recursive = true,
                          const bool use_tollerance = true)
      : recursive_(recursive), use_tollerance_(use_tollerance) {}

  void reorder_edge(const Mesh &mesh, const Integer element_id, Integer &v1,
...
class GloballyUniqueLongestEdgeSelect final : public EdgeSelect<Mesh> {
 public:
  GloballyUniqueLongestEdgeSelect(const Map &map, const bool recursive = true,
                                  const bool use_tollerance = true)
      : map(map),
  ...
  Integer select(const Mesh &mesh, const Integer element_id) const override {
    ...
    std::sort(
        ...
        [](const EdgeDesc &e1, const EdgeDesc &e2) -> bool { return e1 < e2; });
  ...
  void element_refined(const Mesh &mesh, const Integer element_id,
                       const Edge &edge,

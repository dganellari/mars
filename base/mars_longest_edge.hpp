...
namespace mars {
    ...
    class LongestEdgeSelect final : public EdgeSelect<Mesh, Implementation_> {
    public:
        ...
        bool is_recursive() const override { return recursive_; }

        virtual std::string name() const override { return "LongestEdge"; }
    ...
    public:
        ...
        void reorder_edge(const Mesh &mesh, const Integer element_id, Integer &v1, Integer &v2) const override {
            if (v2 < v1) {
                std::swap(v1, v2);
            }
        }
        ...
        virtual std::string name() const override { return "UniqueLongestEdgeSelect"; }
    ...
    public:
        GloballyUniqueLongestEdgeSelect(const Map &map, const bool recursive = true, const bool use_tollerance = true)
            : map(map),
              recursive_(recursive),
              use_tollerance_(use_tollerance)  //, use_trick_(true)
        {}
        ...
        class EdgeDesc {
        public:
            EdgeDesc(const Edge &edge, const Integer edge_num) : edge(edge), edge_num(edge_num) {
                assert(edge.is_valid());
            }

            inline bool operator<(const EdgeDesc &other) const { return edge < other.edge; }
        ...
        Integer select(const Mesh &mesh, const Integer element_id) const override {
            ...
            std::sort(edge_pairs.begin(), edge_pairs.end(), [](const EdgeDesc &e1, const EdgeDesc &e2) -> bool {
                return e1 < e2;
            });
        ...
        virtual std::string name() const override { return "GloballyUniqueLongestEdgeSelect"; }
        ...
        void set_node_rank(const std::shared_ptr<NodeRank> &node_rank) { node_rank_ = node_rank; }

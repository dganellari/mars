...
class ParBisection {
 public:
  ...
  ParBisection(ParMesh &mesh)
      : mesh_(mesh),
        bisection_(mesh.get_serial_mesh()),
        edge_split_pool_(mesh.comm()) {
  ...
  void set_edge_select(const std::shared_ptr<ES> &edge_select) {
    bisection_.set_edge_select(edge_select);
  }
  ...
  void update_edge_pools() {
    edge_split_pool_.update(mesh_, bisection_.edge_element_map(),
  ...
  void green_refinement() {
    ...
    while (!complete) {
      ...
      std::cout << "loop " << loops++ << " done. complete = " << complete
                << std::endl;

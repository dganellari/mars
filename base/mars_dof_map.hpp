...
namespace mars {
    ...
    class Map {
    public:
        Map(const Integer partition_id, const Integer n_partitions)
            : partition_id_(partition_id), n_partitions_(n_partitions) {}

        inline Integer n_partitions() const { return n_partitions_; }
        ...
        inline void push_global(const Integer global_id, const Integer owner = INVALID_INDEX) {
        ...
        inline void add(const Integer local_id, const Integer global_id, const Integer owner = INVALID_INDEX) {
        ...
        inline void set_owner(const Integer local_id, const Integer owner) {
        ...
        inline Integer partition_id() const { return partition_id_; }

        void assign_global_ids(const Integer begin, const Integer end) {
            if (begin == end) return;
        ...
        inline const std::vector<Integer> &global_id() const { return global_id_; }

        void pack_for_global(std::vector<Integer> &buffer, const bool clear_buffer) const {
            if (clear_buffer) {
                buffer.clear();
            }
        ...
        template <class Iter>
        void intersect_partitions(const Iter &begin, const Iter &end, std::vector<Integer> &out) const {
            out.clear();
            if (begin == end) return;
            ...
            std::set_intersection(partitions_[first_it].begin(),
                                  partitions_[first_it].end(),
                                  partitions_[second_it].begin(),
                                  partitions_[second_it].end(),

...
mars::Hex8_Mesh test_mars_hex8_mesh_generation_3D(const int x, const int y, const int z) {
    using namespace mars;
...
mars::Mesh3 test_mars_mesh_generation_3D(const int x, const int y, const int z) {
    using namespace mars;
...
namespace mars {
    ...
    bool test_incomplete(Mesh<Dim, ManifoldDim> &mesh,
        ...
        if (bypass_incomplete) {
            ...
            for (Integer i = 0; i < mesh.n_elements(); ++i) {
                ...
                if (((element_index++ % each_element) == 0) && !edge_select->can_refine(mesh, i)) {
                    // repair global index
    ...
    template <class GlobalEdgeSelect, Integer Dim, Integer ManifoldDim>
    bool test_incomplete(Mesh<Dim, ManifoldDim> &mesh, const bool use_uniform_refinement = false) {
        Map map(0, 1);
        map.resize(mesh.n_nodes(), 0);
    ...
    template <Integer Dim, Integer ManifoldDim>
    void test_incomplete_ND(Mesh<Dim, ManifoldDim> &mesh, const Integer n_tests, const bool use_edge_rank) {
        using MeshD = mars::Mesh<Dim, ManifoldDim>;
        ...
        for (Integer i = 0; i < n_levels; ++i) {
            ...
            if (uniform_refine) {
                ...
                for (Integer k = 0; k < parts.size(); ++k) {
                    ...
                    Vector<Real, Dim> center;
                    center.set(0.5);
                    mark_hypersphere_for_refinement(parts[k]->get_mesh(), center, 0.25, elements[k]);
                    // }
                }

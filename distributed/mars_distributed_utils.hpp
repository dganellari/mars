...
template <std::size_t I = 0, typename... Tp>
    inline typename std::enable_if < I<sizeof...(Tp), void>::type reserve_view_tuple(std::tuple<Tp...>& t, const int size, const std::string desc)
{
...
template <typename F, std::size_t I = 0, typename... Tp>
    inline typename std::enable_if < I<sizeof...(Tp), void>::type apply_impl(const F& f, std::tuple<Tp...>& t, std::tuple<Tp...>& v)
{
...
template <typename F, Integer I = 0, Integer... Args, typename... Tp>
    typename std::enable_if < I<sizeof...(Args), void>::type MARS_INLINE_FUNCTION for_each_arg(const F& f, std::tuple<Tp...>& t)
{
...
struct resize_view_functor {
    resize_view_functor(std::string d, size_t s)
        : _desc(d)
        , _size(s)
    {
    }
...
public:
    ...
    MaxPlus(const ViewVectorType<T> x)
        : x_(x)
    {
    }

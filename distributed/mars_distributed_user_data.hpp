...
template <class Mesh, typename... T> class UserData {
  /* using Mesh = mars::Mesh<Dim, ManifoldDim, DistributedImplementation,
   * NonSimplex<Type, DistributedImplementation>>; */
...
public:
  ...
  MARS_INLINE_FUNCTION void reserve_user_data(user_tuple &tuple,
                                              ...
                                              const Integer size) {
    /* constexpr Integer data_size =
     * std::tuple_size<std::decay<decltype(tuple)>::type>::value; */
    /* reserve_view_tuple(tuple, size, view_desc); */
    apply_impl(resize_view_functor(view_desc, size), tuple);
  }

  template <typename ElementType> struct FillBufferData {
  ...
  MARS_INLINE_FUNCTION void fill_buffer_data(user_tuple &buffer_data) {
    ...
    ViewVectorType<Integer> boundary_lsfc_index =
        host_mesh->get_view_boundary_sfc_index();
    apply_impl(
        fill_buffer_data_functor("fill_buffer_data", size, boundary_lsfc_index),
  ...
  void exchange_ghost_layer(const context &context) {
    ...
    /* auto count_mirror =
       create_mirror_view(host_mesh->get_view_scan_boundary());
  ...
  void print_nth_tuple(const int proc) {
    ...
    parallel_for(
        "print set", ghost_size, KOKKOS_LAMBDA(const Integer i) {
          ...
          double point[3];
          get_vertex_coordinates_from_sfc<simplex_type::ElemType>(
              ghost(i), point, xDim, yDim, zDim);

          Octant o = get_octant_from_sfc<simplex_type::ElemType>(ghost(i));
          printf("ghost data: %li - %li - %li - (%lf, %lf) - data: %lf - proc: "
                 "%li - rank: %i\n",
                 i, ghost(i), elem_index(o.x, o.y, o.z, xDim, yDim), point[0],
                 point[1], data(i), r, proc);
        });
  }

  template <typename ElementType> struct InitialCondition {
    ElementType user_data;
    Integer proc;
  ...
  MARS_INLINE_FUNCTION void init_user_data(T... args) {
    ...
    apply_impl(InitData("init_data", size, std::forward_as_tuple(args...)),
               user_data_);
  }
...
*/
  ...
  template <typename H> MARS_INLINE_FUNCTION void set_init_cond(H f) {
    const Integer size = host_mesh->get_chunk_size();
    Kokkos::parallel_for("init_initial_cond", size, f);
  }
  ...
  template <typename H> MARS_INLINE_FUNCTION void elem_iterate(H f) {
    const Integer size = host_mesh->get_chunk_size();
    Kokkos::parallel_for("elem_iterate", size, f);
  }

  template <typename H> struct FaceIterate {
    ...
    MARS_INLINE_FUNCTION void iterate(const Integer i) const {
      for (int side = 0; side < 2; ++side) {
        ...
        if (face.is_valid() &&
            ((side == 0 && nbh_oc.is_valid()) || ghost || boundary)) {
  ...
  template <typename H> void face_iterate(H f) {
    Integer xDim = host_mesh->get_XDim();
    Integer yDim = host_mesh->get_YDim();
    Integer zDim = host_mesh->get_ZDim();
  ...
  template <std::size_t idx,
            typename H = typename std::tuple_element<idx, user_tuple>::type>
  ...
  // does ont perform a deep copy of the view containted in the mesh. Just the
  // mesh object.
  void copy_mesh_to_device() {
    ...
    Kokkos::parallel_for(
        "CreateDistributedMeshObject", 1, KOKKOS_LAMBDA(const int &) {
          // two local copies for m and tmp since this->m, this->mesh host
          // pointers
          new ((Mesh *)tmp) Mesh(mCopy);
          if (oldDeviceMesh)
            oldDeviceMesh->~Mesh();
          // it only works on a copy since **m is still a host pointer and fails
          // on the device.
        });

    mesh = tmp; // make the mesh pointer a device one so that this init func is
...
void exchange_ghost_user_data(const context &context, UserData &data) {
  ...
  if (data.get_view_scan_recv_mirror()(size) ==
    ...
    errx(1, "Not allowed to call exchange ghost data before the ghost layer "

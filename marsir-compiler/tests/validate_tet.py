#!/usr/bin/env python3
"""Local proof of the MARSIR tet collapsed sum-factorization apply.

Emits mars::marsir::tet_laplacian_apply_host (the generated B^T G_c B sweep) and
compares it against the DENSE per-mode PKD oracle on the same collapsed tet, for
p=2..5. Reproduces rung 9 (see marsir-compiler/TET_SUMFAC_HANDOFF.md):
...
int main() {
    ...
    for (int D = 2; D <= 5; ++D) {
        ...
        // GENERATED apply from MARSIR.
        auto Agen=[&](const std::vector<double>& u, std::vector<double>& y){
            mars::marsir::tet_laplacian_apply_host(D, A, Ad, B, Bd, C, Cd, Gc, u, y);
        };
...
def main():
    if not os.path.isdir(ORACLE):
        print(f"ERROR: tet reference not found at {ORACLE}\n"
              f"Set MARSIR_TET_ORACLE to the tet_ho_ref dir.", file=sys.stderr)
        sys.exit(2)

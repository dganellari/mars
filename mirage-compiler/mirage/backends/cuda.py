...
The generated banks/symbols are namespaced (mars::mirage, mirage_c_*) so the generated
...
def _jvp_block(ea: ElementApply) -> str:
    ...
    L = [
        ...
        "    extern __shared__ char mirage_jvp_smem_raw[];",
        "    RealType* u_sh  = reinterpret_cast<RealType*>(mirage_jvp_smem_raw);",
    ...
    if p_u:
        L.append("                    RealType uq  = my_u[mirage_idx<N, NN>(dir, q, s, r)];")
    if d_interp or d_deriv:
        L.append("                    RealType duq = my_du[mirage_idx<N, NN>(dir, q, s, r)];")
    if p_interp:
        L.append("                    bi  += mirage_c_Btil[l * N + q] * uq;")
    if p_deriv:
        L.append("                    di  += mirage_c_Dtil[l * N + q] * uq;")
    if d_interp:
        L.append("                    bid += mirage_c_Btil[l * N + q] * duq;")
    if d_deriv:
        L.append("                    did += mirage_c_Dtil[l * N + q] * duq;")
    L.append("                }")
    if p_interp:
    ...
    if p_dt2:
        L += ["                RealType dt2 = 0;", "                #pragma unroll",
              "                for (int q = 0; q < N; ++q) dt2 += mirage_c_D[r * N + q] * fA[s * NP + q];"]
    if p_dt1:
        L += ["                RealType dt1 = 0;", "                #pragma unroll",
              "                for (int q = 0; q < N; ++q) dt1 += mirage_c_D[s * N + q] * fA[q * NP + r];"]
    if d_dt2:
        L += ["                RealType dt2_dot = 0;", "                #pragma unroll",
              "                for (int q = 0; q < N; ++q) dt2_dot += mirage_c_D[r * N + q] * fB[s * NP + q];"]
    if d_dt1:
        L += ["                RealType dt1_dot = 0;", "                #pragma unroll",
              "                for (int q = 0; q < N; ++q) dt1_dot += mirage_c_D[s * N + q] * fB[q * NP + r];"]
    if metric:
    ...
    L += [
        ...
        "                RealType v = 0;",
        "                #pragma unroll",
        "                for (int q = 0; q < N; ++q) v += mirage_c_W[r * N + q] * fC[s * NP + q];",
        "                fA[s * NP + r] = v;",
        "            }",
        "            __syncthreads();",
        "            for (int slot = laneInElem; slot < NN; slot += threadsPerElem) {",
        ...
        "                RealType intf = 0;",
        "                #pragma unroll",
        "                for (int q = 0; q < N; ++q) intf += mirage_c_W[s * N + q] * fA[q * NP + r];",
        "                my_y[mirage_idx<N, NN>(dir, l,     s, r)] -= intf;",
        "                my_y[mirage_idx<N, NN>(dir, l + 1, s, r)] += intf;",
        "            }",
        "            __syncthreads();",
        "        }",
        "    if (e < numElements) {",
        ...
        "         int BlockSize = MirageLaunchDefault<P>::Block,",
        "         int ElemsPerBlock = MirageLaunchDefault<P>::Elems>",
        f"inline MIRAGE_gpuError_t {launch2}(const RealType* d_u, const RealType* d_du, RealType* d_y,",
        "                             const int* d_elemDof, const double* d_G,",
        "                             size_t numElements, MIRAGE_gpuStream_t stream = 0,",
...
def _launch_table() -> str:
    lines = ["template<int P> struct MirageLaunchDefault;"]
    for p, (block, elems) in sorted(LAUNCH_DEFAULT.items()):
        lines.append(
            f"template<> struct MirageLaunchDefault<{p}> "
...
def emit(ea: ElementApply) -> str:
    ...
    s1.append("                        RealType uq = my_u[mirage_idx<N, NN>(dir, q, s, r)];")
    if ea.needs_tangential:
        s1.append("                        bi += mirage_c_Btil[l * N + q] * uq;")
    if ea.needs_deriv:
        s1.append("                        di += mirage_c_Dtil[l * N + q] * uq;")
    s1.append("                    }")
    if ea.needs_tangential:
    ...
    if ea.needs_dt2:
        ...
        s2.append("                    for (int q = 0; q < N; ++q) dt2 += mirage_c_D[r * N + q] * faceA[s * NP + q];")
    if ea.needs_dt1:
        s2.append("                    RealType dt1 = 0;")
        s2.append("                    #pragma unroll")
        s2.append("                    for (int q = 0; q < N; ++q) dt1 += mirage_c_D[s * N + q] * faceA[q * NP + r];")
    if ea.needs_metric:
...
#define MIRAGE_gpuError_t          hipError_t
#define MIRAGE_gpuSuccess          hipSuccess
#define MIRAGE_gpuStream_t         hipStream_t
#define MIRAGE_gpuMemcpyToSymbol   hipMemcpyToSymbol
#define MIRAGE_gpuGetLastError     hipGetLastError
#define MIRAGE_gpuFuncSetAttribute hipFuncSetAttribute
#define MIRAGE_gpuMaxDynamicSmem   hipFuncAttributeMaxDynamicSharedMemorySize
#else
#include <cuda_runtime.h>
#define MIRAGE_gpuError_t          cudaError_t
#define MIRAGE_gpuSuccess          cudaSuccess
#define MIRAGE_gpuStream_t         cudaStream_t
#define MIRAGE_gpuMemcpyToSymbol   cudaMemcpyToSymbol
#define MIRAGE_gpuGetLastError     cudaGetLastError
#define MIRAGE_gpuFuncSetAttribute cudaFuncSetAttribute
#define MIRAGE_gpuMaxDynamicSmem   cudaFuncAttributeMaxDynamicSharedMemorySize
#endif
#include <cstddef>
#include <type_traits>

#ifndef MIRAGE_MAX_P
#define MIRAGE_MAX_P 8
#endif
...
namespace detail {{
constexpr int kMaxN  = MIRAGE_MAX_P + 1;
constexpr int kBDlen = MIRAGE_MAX_P * kMaxN;   // Btil / Dtil
...
__constant__ double mirage_c_Btil[detail::kBDlen];
__constant__ double mirage_c_Dtil[detail::kBDlen];
__constant__ double mirage_c_D[detail::kDWlen];
__constant__ double mirage_c_W[detail::kDWlen];

// Upload host HoCvfemOperators arrays once per P before launching.
inline MIRAGE_gpuError_t {up}(int P,
                        const double* h_Btil, const double* h_Dtil,
                        const double* h_D, const double* h_W)
{{
    const int n = P + 1;
    MIRAGE_gpuError_t err;
    err = MIRAGE_gpuMemcpyToSymbol(mirage_c_Btil, h_Btil, sizeof(double) * (size_t)P * n); if (err) return err;
    err = MIRAGE_gpuMemcpyToSymbol(mirage_c_Dtil, h_Dtil, sizeof(double) * (size_t)P * n); if (err) return err;
    err = MIRAGE_gpuMemcpyToSymbol(mirage_c_D,    h_D,    sizeof(double) * (size_t)n * n); if (err) return err;
    err = MIRAGE_gpuMemcpyToSymbol(mirage_c_W,    h_W,    sizeof(double) * (size_t)n * n); if (err) return err;
    return MIRAGE_gpuSuccess;
}}

template<int N, int NN>
__device__ __forceinline__ int mirage_idx(int dir, int nrm, int t1, int t2)
{{
...
{{
    static_assert(std::is_same<RealType, double>::value,
        "mirage HO-CVFEM apply is validated for RealType=double only");
    constexpr int N   = P + 1;
    ...
    extern __shared__ char mirage_smem_raw[];
    RealType* u_sh = reinterpret_cast<RealType*>(mirage_smem_raw);
...
{step2}
            ...
            for (int slot = laneInElem; slot < NN; slot += threadsPerElem) {{
                ...
                RealType v = 0;
                #pragma unroll
                for (int q = 0; q < N; ++q) v += mirage_c_W[r * N + q] * faceB[s * NP + q];
                faceA[s * NP + r] = v;
            }}
            __syncthreads();
            ...
            for (int slot = laneInElem; slot < NN; slot += threadsPerElem) {{
                ...
                RealType intf = 0;
                #pragma unroll
                for (int q = 0; q < N; ++q) intf += mirage_c_W[s * N + q] * faceA[q * NP + r];
                my_y[mirage_idx<N, NN>(dir, l,     s, r)] -= intf;
                my_y[mirage_idx<N, NN>(dir, l + 1, s, r)] += intf;
            }}
            __syncthreads();
...
template<typename RealType, int P,
         int BlockSize = MirageLaunchDefault<P>::Block,
         int ElemsPerBlock = MirageLaunchDefault<P>::Elems>
inline MIRAGE_gpuError_t {launch}(const RealType* d_u, RealType* d_y,
                            const int* d_elemDof, const double* d_G,
                            size_t numElements, MIRAGE_gpuStream_t stream = 0,
                            const int* d_elemList = nullptr, size_t count = 0)
{{
    ...
    MIRAGE_gpuError_t attrErr = MIRAGE_gpuFuncSetAttribute(
        kernel, MIRAGE_gpuMaxDynamicSmem, smemBytes);
    if (attrErr != MIRAGE_gpuSuccess) return attrErr;
